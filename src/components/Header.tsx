...
const Header: React.FC = () => {
  ...
  const isHome = pathname === '/home-page' || pathname === '/';
  ...
  // Anasayfadaysa smooth scroll, değilse anasayfaya yönlendir
  const handleAnchorClick = (e: React.MouseEvent, hash: string) => {
    ...
    if (isHome) {
      ...
      router.push(`/home-page#${hash}`);
    }
  };

  // İndir butonu için
  const handleIndir = (e: React.MouseEvent) => {
    e.preventDefault();
    setMenuOpen(false);
    if (isHome) {
      ...
      router.push('/home-page#indir');
    }
  };

  const navLinks = [
    { label: 'Hakkında', hash: 'features' },
    { label: 'Uygulamalarımız', href: '/apps' },
